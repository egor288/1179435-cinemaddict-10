...
const renderPoster = () => {
  return `<img src="./images/posters/the-man-with-the-golden-arm.jpg" alt="" class="film-card__poster">
          <p class="film-card__description">Frankie Machine (Frank Sinatra) is released from the federal Narcotic Farm in Lexington, Kentucky with a set of drums and a new outlook on…</p>
          <a class="film-card__comments">18 comments</a>
          <form class="film-card__controls">
            <button class="film-card__controls-item button film-card__controls-item--add-to-watchlist">Add to watchlist</button>
...
render(siteMainElement, renderFilms(), `beforeend`);
const filmsContaner = document.querySelector(`.films-list__container`);
render(filmsContaner, renderFilmCard(), `beforeend`);
render(siteMainElement, renderExtraFilms(), `beforeend`);

// for (let i = 0; i <= 5; i++) {
//   render(siteMainElement, renderPoster(), ``);
// }
